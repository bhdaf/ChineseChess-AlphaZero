...
class SupervisedWorker:
    ...
    def __init__(self, config: Config):
        ...
        logger.info(f"监督学习使用设备: {self.device}")
    ...
    def training(self):
        ...
        logger.info(f"开始训练，对局数量 = {len(self.gameinfo)}, 步长 = {self.config.trainer.sl_game_step} 局")
    ...
    def compile_model(self):
        ...
        logger.info("优化器已配置: Adam, 学习率=0.01")
    ...
    def generate_game_data(self, games):
        ...
        end_time = time()
        logger.debug(f"加载 {len(games)} 局对局，用时: {end_time - start_time}s")
